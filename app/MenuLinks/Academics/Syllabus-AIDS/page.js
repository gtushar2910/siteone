...
const SyllabusHome = () => {
  ...
  const [selectedKeys, setSelectedKeys] = useState(new Set(["AIDS"])); // default tab
  ...
  const getSubjects = async () => {
    ...
    const sem = selectedValue === "AIDS" ? "AIDS" : selectedValue;
    try {
      const response = await axios.get(
        "/api/subjects/getSemesterSubjectsAIDS?semester=" + encodeURIComponent(sem)
      );
