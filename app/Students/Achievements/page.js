...
const ForeignStudentsList = () => {
    ...
    const columns = [
        ...
        { name: "ACHIEVEMENT", uid: "achievement" },
    ];

    const [list, setList] = useState([])

    const getList = async () => {
        const response = await axios.get("/api/students/crud/r/getStudentAchievements");
        if (response)
    ...
    return (
        <div className="px-4 cardAboutDept " >
            <div className="box-border p-4 border-0 px-4">
                <h1 className="font-sans text-4xl text-zinc-700 font-black uppercase text-center"> ----- Student Achievements -----</h1>
            </div>
